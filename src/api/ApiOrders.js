...
export async function getByOrderAssignment(asignateto){
    const result = await fetch( path + 'all/assignment', {
        headers: { "content-type": "application/json",
        "authorization": `Bearer ${token}`
        },
        method: "POST",
        body: JSON.stringify({ asignateto })
    }).then(res => res.json())
    return result
}
...
export async function updateOrder(data){
    ...
    const result = await fetch( path + 'edit/' + data._id, {
        headers: { "content-type": "application/json",
        "authorization": `Bearer ${token}`
        },
        method: "PUT",
